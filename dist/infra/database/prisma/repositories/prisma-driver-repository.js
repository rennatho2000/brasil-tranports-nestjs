...
const prisma_Driver_mapper_1 = require("../mappers/prisma-Driver-mapper");
let PrismaDriverRepository = class PrismaDriverRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async create(driver) {
        const raw = prisma_Driver_mapper_1.PrismaDriverMapper.toPrisma(driver);
        await this.prisma.rawDriver.create({
            data: raw,
        });
    }
    async findByTuckId(truckId) {
        const raw = await this.prisma.rawDriver.findFirst({
            where: {
                truckId,
            },
        });
        return raw ? prisma_Driver_mapper_1.PrismaDriverMapper.toDomain(raw) : null;
    }
};
PrismaDriverRepository = __decorate([
